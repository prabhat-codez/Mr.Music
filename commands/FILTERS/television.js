...
if (!args[0]) {
			...
			const msg = await message.channel.send(`Turning on **Television mode**.`);
	...
	player.setEQ(Array(13).fill(0).map((n, i) => ({
        ...
        const msg = await message.channel.send(`Turning off **television mode**.`);
			const embed = new MessageEmbed()
				.setDescription('Turned off **television mode**')
				.setColor("BLUE");
			await delay(5000);
			return msg.edit('', embed);
		}
