...
run: async (client, message, args) => {
 ...
 let player = client.manager.players.get(message.guild.id);
		...
		if (!message.member.voice.channel.permissionsFor(message.guild.me).has('CONNECT')) {
			return embed("I need permission to **connect** to yourvoice channel", message.channel).then(m => m.delete({ timeout: 10000 }));
		}
		...
		embed("Successfully connected to your voice channel",message.channel)
			...
			try {
				...
				const embed = new MessageEmbed()
					.setColor("GREEN")
					.setDescription("Successfully joined your voice channel");
